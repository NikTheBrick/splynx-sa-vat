                ...
                foreach ($all_services as &$serv) {
                    ...
                    if ($serv['status'] == 'active') {
                        if ($serv['date'] < '2025-05-01'){
                            ...
                            if ($result) {
                                file_put_contents($log_file,  "Old service ID: {$serv['id']} -> New {$type} service updated to service ID: {$api->response} \n", FILE_APPEND);
                            } else {
                            ...
                            if ($result) {
                                file_put_contents($log_file,  "Active service ID: {$serv['id']} has been updated -> Ok \n", FILE_APPEND);
                            } else {
                                file_put_contents($log_file,  "Active service ID: {$serv['id']} updating ERROR: " . print_r($api->response, true) . "\n", FILE_APPEND);
                            }
                        }
